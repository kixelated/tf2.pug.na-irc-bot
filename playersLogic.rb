module PlayersLogic
  def add_player user, classes
    ...
    classes.reject! { |clss| not Variables::Team_classes.key? clss }
    classes.uniq!
  ...
  def list_players_detailed
    temp = get_classes
    Variables::Team_classes.each_key do |k|
  ...
  def classes_needed players, multiplier = 1
    required = Variables::Team_classes.collect { |k, v| v * multiplier - (players[k] ||= []).size }
  ...
  def list_classes_needed
    ...
    output.unshift [ "players" , (Variables::Team_size * Variables::Team_count - @players.size)] if @players.size < Variables::Team_size * Variables::Team_count
  ...
  def minimum_players?
    return false if @players.size < Variables::Team_size * Variables::Team_count
