...
public Plugin:myinfo = {
	name = "tf2.pug.na - Restricter",
	author = "Jean-Denis Caron, Luke Curley",
	description = "Restricts off-classing using the !restrict command.",
...
public Action:checkForOffClassPlayers(Handle:timer) {
  ...
  for (new i = 1; i <= playerCount; i++) {
    ...
    if (IsValidEntity(i) && IsClientInGame(i)) {
      ...
      // TODO: Wut. Confirm this is correct.
      if (classID != 1 && classID != 3 && classID != 4 && classID != 5) {
        if (team == 2) {
          ++teamAOffClassPlayers;
        } else if (team == 3) {
          ++teamBOffClassPlayers;
        }
      }
    }
    
    new teamWithOffClassPlayers = 0; // 0 = nil, 1 = A, 2 = B, 3 = AB
    if (teamAOffClassPlayers > 1) { teamWithOffClassPlayers += 1; } // First bit flag
    if (teamBOffClassPlayers > 1) { teamWithOffClassPlayers += 2; } // Second bit flag
    
    // TODO: Finish file (up to this point)
